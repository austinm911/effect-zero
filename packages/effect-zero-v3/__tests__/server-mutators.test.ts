...
describe("Effect v3 server mutator helpers", () => {
  ...
  test("composes a shared mutator with a service-backed server override and deferred post-commit work", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v3_server_mutators",
    });
    ...
    class CartMutationWorkflow extends Effect.Service<CartMutationWorkflow>()(
      "test/CartMutationWorkflow",
      {
        effect: Effect.sync(() => ({
          plan: (input: { readonly albumId: string; readonly userId: string }) =>
            Effect.sync(() => {
    ...
    const handler = createServerMutatorHandler({
      ...
      executeEffect: ({ effect }) =>
        Effect.runPromise(
          Effect.provide(effect, CartMutationWorkflow.Default) as Effect.Effect<any, any, never>,
        ),
    });
  ...
  test("surfaces an app error when a server override calls runDefaultMutation twice", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v3_server_mutators",
    });
    const seed = await testDatabase.seedBaseMusicRows();
  ...
  test("preserves duplicate replay semantics through the v3 transaction bridge", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v3_server_mutators",
    });
    const seed = await testDatabase.seedBaseMusicRows();
  ...
  test("preserves out-of-order push failures through the v3 transaction bridge", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v3_server_mutators",
    });
    const seed = await testDatabase.seedBaseMusicRows();
