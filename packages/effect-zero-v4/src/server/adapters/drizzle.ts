...
> = EffectPgDatabase<TRelations> & { readonly __effectZeroSchema?: TSchema };
...
> {
  ...
  readonly drizzleConfig?: Omit<EffectDrizzlePgConfig<TRelations>, "relations">;
  ...
  implements DBTransaction<TWrappedTransaction>, EffectV4TransactionEffectRunner
{
  ...
  readonly query: DBTransaction<TWrappedTransaction>["query"] = async (sql, params) => {
    ...
    ).prepareQuery({ params, sql }, "objects", false);
...
>(options: CreateDbConnectionOptions<TSchema, TRelations>) {
  ...
  try {
    ...
    const drizzleDatabase = await runtime.runPromise(
      PgDrizzle.makeWithDefaults<TRelations>({
