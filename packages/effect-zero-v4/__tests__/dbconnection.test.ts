...
describe("Effect v4 DBConnection", () => {
  ...
  test("supports Zero ZQL reads, raw SQL, and native Effect Drizzle transaction access", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v4",
    });
    ...
    try {
      ...
      const resultDrizzle = await zql.transaction(
        (tx) =>
          Effect.runPromise(
            tx.dbTransaction.wrappedTransaction.query.artist.findFirst({
              where: {
                id: seed.artist.id,
              } as never,
            }),
  ...
  test("exposes a transaction-scoped Effect runner on both dbTransaction and wrappedTransaction", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v4",
    });
    const seed = await testDatabase.seedBaseMusicRows();
  ...
  test("runs handleMutateRequest through a Zero provider backed by the Effect v4 adapter", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v4",
    });
    const seed = await testDatabase.seedBaseMusicRows();
  ...
  test("wraps a node-postgres pool with raw SQL and native client transaction access", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v4_pg",
    });
    const seed = await testDatabase.seedBaseMusicRows();
  ...
  test("wraps a postgres.js client with raw SQL and tagged-template transaction access", async () => {
    testDatabase = await createTestDatabase({
      databaseNamePrefix: "effect_zero_v4_postgresjs",
    });
    const seed = await testDatabase.seedBaseMusicRows();
  ...
  test("exposes the native Effect Drizzle client for direct writes", async () => {
    ...
    try {
      await Effect.runPromise(
        connection.drizzle.execute(sql`
