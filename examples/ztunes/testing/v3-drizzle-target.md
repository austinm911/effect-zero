# Effect v3 Drizzle Target

Verify the browser-visible `v3-drizzle` target end to end at `http://effect-zero-ztunes.localhost:1355`.

## Assertions

1. Home route loads and shows the target tabs plus the artist search input.
2. Clicking the `Effect v3 (Drizzle)` tab sets `document.cookie` so `effect-zero-target=v3-drizzle`.
...
3. Use `click` or `eval` to activate the `Effect v3 (Drizzle)` button.
4. Use `eval` to confirm `document.cookie.includes("effect-zero-target=v3-drizzle")`.
